# Keybindings
Mecro has a plethora of hotkeys that make it easy and powerful to use and all
hotkeys are fully customizable to your liking.
Custom keybindings are stored internally in mecro if changed with the `> bind`
command or can also be added in the file `~/.config/mecro/bindings.json` as
discussed below. For a list of the default keybindings in the json format used
by mecro, please see the end of this file. For a more user-friendly list with
...
If `~/.config/mecro/bindings.json` does not exist, you can simply create it.
Mecro will know what to do with it.
...
## Rebinding keys
The bindings may be rebound using the `~/.config/mecro/bindings.json` file.
...
In addition to editing your `~/.config/mecro/bindings.json`, you can run
...
mecro won't immediately execute the command when you press the binding, but
...
starting with `0x1b`.
For example, if mecro reads `\x1b[1;5D`, on most terminals this will mean the
...
sends for `Backspace` meaning mecro can't bind `CtrlBackspace`.
However, some terminals do allow you to bind keys to send specific escape
sequences you define. Then from mecro you can directly bind those escape
...
`Alt-Up`. Mecro supports an optional `-` between modifiers like `Alt` and
`Ctrl` so `Alt-a` could be rewritten as `Alta` (case matters for alt bindings).
This is why in the default keybindings you can see `AltShiftLeft` instead of
`Alt-ShiftLeft` (they are equivalent).
Please note that terminal emulators are strange applications and mecro only
receives key events that the terminal decides to send. Some terminal emulators
may not send certain events even if this document says mecro can receive the
event. To see exactly what mecro receives from the terminal when you press a
