Copy and paste are essential features in mecro but can be
confusing to get right especially when running mecro over SSH
...
* If you are not working over SSH, use the mecro keybinding (`Ctrl-v`
...
## Mecro paste events
Mecro is an application that runs within the terminal. This means
that the terminal sends mecro events, such as key events, mouse
events, resize events, and paste events. Mecro's default keybinding
for paste is `Ctrl-v`. This means that when mecro receives the key
...
mecro and will instead do something itself. In this document,
...
mecro either a paste event or a list of key events (one key for each
...
sends a list of key events, this can cause issues because mecro
...
while you perform the paste. When paste option is on, mecro will
...
entered.
## Pasting over SSH
When working over SSH, mecro is running on the remote machine and
your terminal is running on your local machine. Therefore if you
would like to paste, using `Ctrl-v` (mecro's keybinding) will not
work because when mecro attempts to access the system clipboard,
...
* If you are not working over SSH, use the mecro keybinding (`Ctrl-c` by
...
Mecro has a keybinding (`Ctrl-c`) for copying and will access the system
clipboard to perform the copy. The text that mecro will copy into is
the text that is currently selected in mecro (usually such text is
...
selection (*not* mecro's selection). To select text with the terminal
selection, mecro's mouse support must first be disabled by turning the
`mouse` option off. The terminal, unlike mecro, has no sense of different
